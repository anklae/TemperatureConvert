...
class QuizProgam():
    def __init__(self):
        ...
        self.YellowButton = Button(self.button_frame, text= "Yellow", bg= self.color, fg= "#000000", relief=RIDGE, width=12)
        self.YellowButton.grid(row=4, column = 2, padx=3, pady=5)
        self.my_button = Button(root, text="Pick a Colour", command=self.color)
        self.my_button.grid(row=5,column=1)
    
    def color(self):
      self.my_color = colorchooser.askcolor()[1]
       
          
          #color_label2 = Label(root, text ="You picked a color!", font=("Helvetica, 32"), bg = my_color).pack()
